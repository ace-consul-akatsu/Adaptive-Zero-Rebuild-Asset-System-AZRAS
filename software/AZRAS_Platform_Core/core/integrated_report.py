...
from core.branding import DISPLAY_NAME, COMPANY_EN, COMPANY_JA, COPYRIGHT, GITHUB, LICENSE_EN, LICENSE_JA
...
def _report_story(project: dict[str, Any], source_path: Path | None = None) -> list[Any]:
    ...
    story += [
        PageBreak(),
        _p("Software Information / ソフトウェア情報", s["h1"]),
        _table([
            ["Product / 製品", DISPLAY_NAME],
            ["Developer / 開発・提供", f"{COMPANY_EN} / {COMPANY_JA}"],
            ["Copyright", COPYRIGHT],
            ["Repository", GITHUB],
        ], [52*mm, 123*mm], header=False),
        Spacer(1, 6*mm),
        _p("License and Disclaimer", s["h2"]),
        _p(LICENSE_EN.replace("\n", "<br/>"), s["body"]),
        Spacer(1, 4*mm),
        _p("ライセンス・免責事項", s["h2"]),
        _p(LICENSE_JA.replace("\n", "<br/>"), s["body"]),
    ]
    return story
...
class _ReportDoc(BaseDocTemplate):
    ...
    def _footer(self, canvas, doc):
        ...
        canvas.drawString(16 * mm, 9 * mm, f"{DISPLAY_NAME} — Official Report — Page {doc.page}")
        canvas.drawRightString(A4[0] - 16 * mm, 9 * mm, COPYRIGHT)
        canvas.restoreState()
