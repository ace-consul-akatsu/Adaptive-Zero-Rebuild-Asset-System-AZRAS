...
def _area(p,key):
    c=p.get('common') or {}; b=c.get('building') or {}; return _f(c.get(key),_f(b.get({'scale_gfa_m2':'gross_floor_area_m2','roof_area_m2':'roof_area_m2'}[key])))
...
def build_case(base,loc,base_file):
    ...
    analysis={'source_type':'ai_estimate','confidence':0.46,'basis':['base_project_json','latitude','longitude','gross_floor_area','coarse_climate_heuristic'],'gross_floor_area_m2':gfa,'estimated_annual_heating_energy_kWh':round(gfa*_f(climate['heating_degree_days_estimate'])*.012,1),'estimated_annual_cooling_energy_kWh':round(gfa*_f(climate['cooling_degree_days_estimate'])*.010,1),'estimated_annual_pv_generation_kWh':round(pv,1)}
